...
impl Operand{
    ...
    pub(crate) fn map_value_from_string(value:&String,constraint: Constraint) -> Result<OperandValue> {
        ...
        match constraint {
            ...
            Constraint::w => {
                value.remove(0);
                Ok(OperandValue::new(u32::from_str(&*value)?))
            }
            ...
            Constraint::E|
            Constraint::j=> {
                Ok(OperandValue::new(u32::from_str_radix(value.trim_start_matches("0X"), 16)?))
            }
            Constraint::l|
            Constraint::L => {
                value.remove(0);
                Ok(OperandValue::new(i32::from_str(&*value)?))
            }
            Constraint::o => {
                value.remove(0);
                Ok(OperandValue::new(u32::from_str(&*value)?))
...
fn pointer_into_constraint_e(val:&str) ->Result<OperandValue>{
    ...
    match val.to_uppercase().chars().next().unwrap() {
        'X' => Ok(OperandValue::new(3u32)),
        'Y' => Ok(OperandValue::new(2u32)),
        'Z' => Ok(OperandValue::new(0u32)),
...
fn pointer_into_constraint_b(val:&str) ->Result<OperandValue>{
    ...
    match val.to_uppercase().as_str() {
        "" => Ok(OperandValue::new(0u32)),
        "Z+" => Ok(OperandValue::new(1u32)),
        _ => Err(Error::InvalidValue),
    }
}
